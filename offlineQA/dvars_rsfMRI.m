function [dvars] = GetDVARS(rsData,MaskIn,opts)
    ...
    switch opts.normalization
    	...
    	% First look at using regression
    if(~isempty(opts.regressor))    
		ts = RegressNoiseSignal(ts.',opts.regressor).';
    end
...
%     keyboard
	...
	% ------------------------------------------------------------------------------
	% Mask out left over bad mask voxels -- can happen sometimes
	% ------------------------------------------------------------------------------
	nonZeroInds = find(ts(1,:) ~= 0);
	ts = ts(:,nonZeroInds);
